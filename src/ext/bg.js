...
import { DownloadMan, getDownloadMan } from "../common/download_man";
import {
  SIDEPANEL_PORT_NAME,
  SIDEPANEL_TAB_ID,
  bgInit,
} from "../common/ipc/ipc_bg_cs";
import { getIpcCache } from "../common/ipc/ipc_cache";
import log from "../common/log";
import { getWindowSize, resizeViewportOfTab } from "../common/resize_window";
...
import {
  ...
  getTab,
  updateUrlForTab,
} from "../common/tab_utils";
import { singletonGetter, singletonGetterByKey } from "../common/ts_utils";
import {
  and,
  delay,
  ensureExtName,
...
import { getProxyManager, setProxy } from "../services/proxy";
...
const getToplayTabId = async () => {
  return new Promise((resolve, reject) => {
    return Ext.tabs.query({ active: true }).then(async (tabs) => {
      resolve(tabs[0]);
    });
  });
};
...
const showBadge = (options) => {
  ...
  if (blink) {
    setTimeout(() => {
      Ext.action.getBadgeText({}).then((curText) => {
...
const notifyRecordCommand = (command) => {
  ...
  setTimeout(() => {
    Ext.notifications.clear(notifId).catch((e) => log.error(e));
  }, 2000);
};
...
const isTimeToBackup = () => {
  return storage.get("config").then((config) => {
    const { enableAutoBackup, lastBackupActionTime, autoBackupInterval } =
      config;
    ...
    .then(([tab, state]) => {
      ...
      switch (state.status) {
        case C.APP_STATUS.NORMAL:
          return Ext.windows.get(tab.windowId).then((win) => win.focused);
        ...
        default:
          throw new Error(
            `E213: isTabActiveAndFocused: unknown app status, '${state.status}'`
          );
      }
    })
    .catch((e) => false);
...
const getCurrentStorageManager = () => {
  ...
  return Promise.all([restoreConfig(), getXFile().getConfig()]).then(
    ([config, xFileConfig]) => {
...
const bindEvents = () => {
  ...
  Ext.tabs.onRemoved.addListener(async (tabId, removeInfo) => {
    ...
    if (state.status === C.APP_STATUS.PLAYER && tabId === state.tabIds.toPlay) {
      ...
      return Ext.windows
        .get(removeInfo.windowId, { populate: true })
        .then((win) => {
          const pActiveTab = !win
            ...
            : Promise.resolve(win.tabs.find((tab) => tab.active));
  ...
  const getCalculatedShowSidePanelValue = (config) => {
    ...
    if (config) {
      if (
        config.oneTimeShowSidePanel &&
        [true, false].includes(config.oneTimeShowSidePanel)
      ) {
  ...
  if (Ext.isFirefox()) {
    storage.addListener(([storage]) => {
      if (storage.key === "config") {
        ...
        if (
          storage.newValue.oneTimeShowSidePanel !==
            storage.oldValue.oneTimeShowSidePanel &&
          [true, false].includes(storage.newValue.oneTimeShowSidePanel)
        ) {
  ...
  Ext.windows.onFocusChanged.addListener((windowId) => {
    ...
    Ext.tabs.query({ windowId, active: true }).then((tabs) => {
        ...
        getIpcCache()
          ...
          .then((state) => {
            ...
            return Promise.all([
              ...
              await updateState(
                setIn(["tabIds", "toRecord"], activeInfo.tabId)
              );
          ...
          chrome.downloads.search(
            ...
            function (downloadItems) {
              if (downloadItems && downloadItems.length > 0) {
                console.log(
                  "Downloaded file name111: " + downloadItems[0].filename
                );
...
const onRequest = async (cmd, args) => {
  ...
  switch (cmd) {
    ...
    case "I_AM_PANEL":
      ...
      let isSidePanel =
        args.sender.tab?.id === SIDEPANEL_TAB_ID ||
        args.sender.url ===
          `chrome-extension://${Ext.runtime.id}/sidepanel.html` ||
      ...
      if (
        args.sender.tab &&
        args.sender.tab.id &&
        state.tabIds.toPlay === args.sender.tab.id
      ) {
        await updateState((state) => ({
          ...state,
          tabIds: {
            ...
            lastActivated: state.tabIds.lastActivated.filter(
              (id) => id !== args.sender.tab.id
            ),
          },
        }));
      }
    ...
    case "PANEL_CAPTURE_VISIBLE_TAB": {
      return Ext.tabs
        .captureVisibleTab(args.windowId, args.options)
        .catch((e) => {
          console.log("captureVisibleTab e:>>", e);
          if (e == "Error: Missing activeTab permission") {
            throw new Error(
              "Error E144: Screenshot permission issue. To fix, please reload extension." +
    ...
    case "PANEL_START_RECORDING": {
      ...
      const menuInfos = [
        ...
        ...item,
        onclick: () => {
          getRecordTabIpc().then((ipc) =>
            ipc.ask("CONTEXT_MENU_IN_RECORDING", { command: item.id })
          );
        },
      }));
      ...
      const list = state.tabIds.lastActivated.filter(
        (id) => id !== state.tabIds.panel
      );
      const lastActivatedTabId = list[list.length - 1];

      if (lastActivatedTabId) {
        activateTab(lastActivatedTabId, true).catch((e) => {
    ...
    case "PANEL_TRY_TO_RECORD_OPEN_COMMAND": {
      ...
      return getPlayTab().then(async (tab) => {
    ...
    case "PANEL_HIGHLIGHT_RECT": {
      return getPlayTabIpc().then((ipc) =>
        ipc.ask("HIGHLIGHT_RECT", args, C.CS_IPC_TIMEOUT)
      );
    }

    case "PANEL_HIGHLIGHT_X": {
      return getPlayTabIpc().then((ipc) =>
        ipc.ask("HIGHLIGHT_X", args, C.CS_IPC_TIMEOUT)
      );
    }
    ...
    case "PANEL_HIGHLIGHT_RECTS": {
      ...
      return getPlayTabIpc().then((ipc) =>
        ipc.ask("HIGHLIGHT_RECTS", args, C.CS_IPC_TIMEOUT)
      );
    }
    ...
    case "PANEL_HIGHLIGHT_OCR_MATCHES": {
      if (args.isDesktop) {
        return getCurrentStorageManager().then((storageManager) => {
          const source =
            storageManager.getCurrentStrategyType() ===
            StorageStrategyType.XFile
        ...
        return getPlayTabIpc().then((ipc) =>
          ipc.ask("HIGHLIGHT_OCR_MATCHES", args, C.CS_IPC_TIMEOUT)
        );
      }
    }
    ...
    case "PANEL_UPDATE_BADGE": {
      ...
      if (!fn) {
        throw new Error(
          `E221: unknown type for updating badge, '${args.type}'`
        );
      }
    ...
    case "PANEL_MINIMIZE_ALL_WINDOWS_BUT_PANEL": {
      const pPanelTab = !state.tabIds.panel
        ? Promise.resolve()
        : Ext.tabs.get(state.tabIds.panel);
      ...
      return Promise.all([pPanelTab, pAllWindows])
        .then(([tab, wins]) => {
          const list = !tab
            ? wins
            : wins.filter((win) => win.id !== tab.windowId);
          return Promise.all(
            list.map((win) =>
              Ext.windows.update(win.id, { state: "minimized" })
            )
          );
        })
        .then(() => delay(() => true, 500));
...
*/
    ...
    case "PANEL_RESIZE_PLAY_TAB": {
      return getPlayTab().then((tab) =>
        resizeViewportOfTab(tab.id, args.viewportSize, args.screenAvailableRect)
      );
    }
    ...
    case "PANEL_SELECT_AREA_ON_CURRENT_PAGE": {
      return getPlayTabIpc()
        ...
        .catch((e) => {
          log.error(e.stack);
          throw new Error(
            "E205: Not able to take screenshot on the current tab"
          );
        });
    }
    ...
    case "PANEL_HIDE_VISION_HIGHLIGHT": {
      return getPlayTabIpc().then((ipc) =>
        ipc.ask("HIDE_VISION_RECTS", {}, C.CS_IPC_TIMEOUT)
      );
    }

    case "PANEL_SHOW_VISION_HIGHLIGHT": {
      return getPlayTabIpc().then((ipc) =>
        ipc.ask("SHOW_VISION_RECTS", {}, C.CS_IPC_TIMEOUT)
      );
    }
    ...
    case "PANEL_TOGGLE_HIGHLIGHT_VIEWPORT": {
      return getPlayTabIpc().then((ipc) =>
        ipc.ask("TOGGLE_HIGHLIGHT_VIEWPORT", args, C.CS_IPC_TIMEOUT)
      );
    }
    ...
    case "PANEL_GET_VIEWPORT_RECT_IN_SCREEN": {
      return Promise.all([
        ...
        return getPlayTabIpc().then((ipc) =>
          ipc.ask("GET_VIEWPORT_RECT_IN_SCREEN", { zoom })
        );
      });
    }
    ...
    case "PANEL_XCLICK_NEED_CALIBRATION": {
      ...
      const isWindowInfoEqual = (a, b) => {
        return and(
          ..."id, top, left, width, height, activeTabId"
            .split(/,\s*/g)
            .map((key) => a[key] === b[key])
        );
      };
      ...
      return getPlayTab().then((tab) => {
        ...
        return Ext.windows.get(tab.windowId).then(async (win) => {
    ...
    case "PANEL_CLOSE_CURRENT_TAB_AND_SWITCH_TO_LAST_PLAYED": {
      return getPlayTab().then((currentTab) => {
        return Ext.windows
          .get(currentTab.windowId, { populate: true })
          .then(async (win) => {
    ...
    case "PANEL_CLOSE_CURRENT_TAB": {
      ...
      const withKantuWindowMinimized = (fn) => {
        const getPanelWinId = () =>
          Ext.tabs.get(state.tabIds.panel).then((tab) => tab.windowId);
        const minimize = () =>
          getPanelWinId().then((winId) =>
            Ext.windows.update(winId, { state: "minimized" })
          );
        const restore = () =>
          getPanelWinId().then((winId) =>
            Ext.windows.update(winId, { state: "normal" })
          );
              ...
              .then((win) => {
                ...
                return getCurrentTab().then((tab) => {
                  if (tab && tab.id !== state.tabIds.panel) {
                    return closeTabAndGetNextTabOnWindow().then((tab) => {
                      if (tab && tab.id === state.tabIds.panel) {
                  ...
                  return withKantuWindowMinimized(
                    closeTabAndGetNextTabOnWindow
                  );
                });
              });
          })
    ...
    case "PANEL_SELECT_WINDOW": {
        ...
        case "tab": {
          if (/^\s*open\s*$/i.test(locator)) {
            pGetTabs = Ext.tabs
              .get(state.tabIds.toPlay)
              .then((tab) =>
                Ext.tabs.create({ url: args.value, windowId: tab.windowId })
              )
              .then((tab) => [tab]);
          } else {
            ...
            pGetTabs = Ext.tabs.get(state.tabIds.firstPlay).then((tab) =>
              Ext.tabs.query({
      ...
      return pGetTabs
        .then((tabs) => {
          if (tabs.length === 0) {
            throw new Error(
              `E210: failed to find the tab with locator '${args.target}'`
            );
          }
          return tabs[0];
        })
        .then((tab) => {
          ...
          return getIpcCache()
            ...
            .then((ipc) => {
              ...
              return ipc
                .ask("DOM_READY", {}, domReadyTimeout)
                .catch((e) => {
                  log.error(e);
                  // most likely, ipc is not running properly in this tab
                  throw new Error(
                    `E226: DOM failed to be ready in ${domReadyTimeout} ms'`
                  );
                })
                .then(() => {
                  ipc.ask("SET_STATUS", {
        ...
        .catch((e) => {
          ...
          return Promise.all([Ext.windows.getCurrent()]).then((window) => {
            return Ext.tabs
              ...
              .then(async (tabs) => {
                ...
                const ctab = tabs.filter(
                  (r) =>
                    r.active === true &&
                    r.url.indexOf("chrome-extension://") == -1
                );
                const offset = parseInt(locator, 10);
                let wt = await checkTaIsPresent(
                  ctab[0].index + offset,
                  tabs[0].windowId
                );
                ...
                if ((tab.index == 0 && offset == 0) || wt != "") {
                  ...
                  throw new Error(
                    `E212: failed to find the tab with locator '${args.target}'`
                  );
                  //log.error(e.stack)
                  //throw e
                }
    ...
    case "CS_INVOKE": {
      return storage.get("config").then(async (config = {}) => {
        ...
        const from =
          (args.testCase && args.testCase.from) ||
          (args.testSuite && args.testSuite.from);

        switch (from) {
          case "bookmark": {
            if (!config.allowRunFromBookmark) {
              throw new Error(
                "[Message from RPA] Error E103: To run a macro or a test suite from bookmarks, you need to allow it in the Ui.Vision settings first"
              );
            }
            break;
          }
          ...
          case "html": {
            ...
            if (isFileSchema && !config.allowRunFromFileSchema) {
              throw new Error(
                "Error #103: To run test suite from local file, enable it in Ui.Vision settings first"
              );
            }

            if (isHttpSchema && !config.allowRunFromHttpSchema) {
              throw new Error(
                "Error #104: To run test suite from public website, enable it in Ui.Vision settings first"
              );
            }
    ...
    case "CS_IMPORT_AND_INVOKE": {
      ...
      return storage.get("config").then((config = {}) => {
        ...
        if (isFileSchema && !config.allowRunFromFileSchema) {
          throw new Error(
            "Error #105: To run macro from local file, enable it in RPA settings first"
          );
        }

        if (isHttpSchema && !config.allowRunFromHttpSchema) {
          throw new Error(
            "Error #105: To run macro from public website, enable it in the RPA settings first"
          );
        }

        return withPanelIpc({ params: { from } }).then((panelIpc) => {
    ...
    case "CS_ADD_LOG": {
      return getPanelTabIpc().then((ipc) => ipc.ask("ADD_LOG", args));
    }
...
const initPlayTab = () => {
  return Promise.all([Ext.windows.getCurrent(), getState()]).then(
    ([window, state]) => {
...
import("../services/supabase-worker")
  .then(({ initSupabaseWorker }) => {
    console.log("📦 Supabase Worker module loaded");
    initSupabaseWorker({
      supabaseUrl: "https://rootomzbucovwdqsscqd.supabase.co",
      supabaseAnonKey:
        "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InJvb3RvbXpidWNvdndkcXNzY3FkIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MzU4OTE4ODMsImV4cCI6MjA1MTQ2Nzg4M30.fYKOe-HPh4WUdvBhEJxakLWCMQBp4E90EDwARk7ucf8",
